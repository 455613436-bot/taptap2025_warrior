...
public class EndStoryControlNew : MonoBehaviour
{
    [Header("动画设置")]
    ...
    // 静态实例，方便其他脚本访问
    public static EndStoryControlNew Instance { get; private set; }
    ...
    private IEnumerator PlayDialogSequence()
    {
        ...
        if (dialog2 != null)
        {
            ...
            // 播放完Dialog2后显示BackScreen（直接显示，无淡入效果）
            ShowBackScreen();
        }
        else
        {
    ...
    private IEnumerator PlayEndImage()
    {
        ...
        // 激活EndImage时立刻隐藏BackScreen
        HideBackScreen();
        Debug.Log("隐藏BackScreen");
        ...
        if (endImage != null)
        {
            ...
            // EndImage淡出结束后立刻激活BackScreen（直接显示，无淡入效果）
            ShowBackScreen();
    ...
    /// <summary>
    /// 显示BackScreen（直接显示，无淡入效果）
    /// </summary>
    private void ShowBackScreen()
    {
        if (backScreen != null)
        {
            ...
            Debug.Log("显示 BackScreen");
        }
        else
        {
            Debug.LogWarning("BackScreen 未设置");
        }
    }
